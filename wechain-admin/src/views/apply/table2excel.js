        ...
        while (i < len) {
            if (i === -1) {
                ...
                Array.from(tableBody.rows[i].children).forEach((td) => {
                    let cellData = '';
                    if (td.children[0].children[0] !== undefined) {
                        cellData = td.children[0].children[0].innerHTML;
                    } else {
                        // 针对type: 'index',（序号列）的情况，td只有一级child（是文本数据），此时使用td.children[0].innerHTML；其它情况td内嵌套有span标签，span内为文本数据，应使用td.children[0].children[0].innerHTML
                        cellData = td.children[0].innerHTML;
                    }
                    tableInnerHTML = tableInnerHTML + '<td>' + cellData + '</td>';
                });
                tableInnerHTML += '</tr>';
            }
            i++;
